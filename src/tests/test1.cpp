...
CMyApplication::addROSpec (void)
{
    ...
    pAISpecStopTrigger->setDurationTrigger(1000);
  ...
  CTagReportData *              pTagReportData)
{
    ...
    //unsigned int antennaId = pTagReportData->getAntennaID();
    
    static unsigned long long previousTimeStamp = 0;
		unsigned long long timeStamp = pTagReportData->getFirstSeenTimestampUTC()->getMicroseconds();
		double lapTime = 0.;
		if (previousTimeStamp > 0) lapTime = (double)(timeStamp - previousTimeStamp) / 1000000.;
		previousTimeStamp = timeStamp;
		//unsigned long long lastTimeSeen = pTagReportData->getLastSeenTimestampUTC()->getMicroseconds();
		//unsigned long long mean = (firstTimeSeen + lastTimeSeen) / 2;
		//unsigned long long interval = lastTimeSeen - firstTimeSeen;
    printf("<%-32s> timeStamp=%llu lapTime=%.3f sec\n", aBuf, timeStamp, lapTime);
